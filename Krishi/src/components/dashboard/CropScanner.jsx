    ...
    <div className="db-section scanner-section">
      <div className="scanner-page-header">
        <div>
          ...
          <p className="db-page-sub">AI-powered plant disease detection — PlantVillage MobileNetV2 ML model + Gemini AI. Scan leaves, fruits, or vegetables.</p>
        </div>
        {(activeImage || result) && (
      ...
      <div className="scanner-main-grid">
        ...
        <div className="scanner-viewport-card">
          ...
          <div style={{ background: 'rgba(34,197,94,0.08)', border: '1.5px solid rgba(34,197,94,0.25)', borderRadius: '10px', padding: '0.6rem 0.9rem', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.78rem', color: 'var(--text)' }}>
            <span>🌱</span>
            <span><strong>Crops, fruits, or vegetables only.</strong> Point camera at leaves, stems, fruits, or affected areas.</span>
          </div>
          ...
          <div className="scanner-viewport">
            {cameraState === 'idle' && !activeImage && (
              <div className="scanner-empty-state">
                ...
                <p className="scanner-empty-sub">Launch camera or upload a leaf, fruit, or vegetable image below.</p>
              </div>
            )}
            ...
            {scanning && (
              <div className="scanner-overlay">
                ...
                <p className="scanner-overlay-text">
                  {scanStep === 1 ? '🧬 Preparing image…' : scanStep === 2 ? '📡 Running PlantVillage ML model…' : '🤖 Generating treatment plan…'}
                </p>
        ...
        <div className="scanner-result-card">
          ...
          {result && (
            <div className="scanner-result-body">
              ...
              {result._note && (
                <div style={{ background: '#fff7ed', border: '2px solid #f97316', borderRadius: '10px', padding: '0.7rem 1rem', marginBottom: '0.85rem', display: 'flex', alignItems: 'flex-start', gap: '0.6rem' }}>
                  <span style={{ fontSize: '1.3rem', flexShrink: 0 }}>⚠️</span>
                  <div>
                    <p style={{ margin: 0, fontWeight: 800, fontSize: '0.8rem', color: '#c2410c', letterSpacing: '0.04em' }}>DEMO MODE — Mock Data Shown</p>
                    <p style={{ margin: '0.2rem 0 0', fontSize: '0.74rem', color: '#9a3412', lineHeight: 1.45 }}>
                      Your Gemini API key is missing or invalid. Real AI diagnosis is disabled.
                      Get a <strong>free key</strong> at{' '}
                      <a href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer"
                        style={{ color: '#ea580c', fontWeight: 700 }}>aistudio.google.com/apikey</a>
                      {' '}(starts with <code style={{ background: '#fed7aa', padding: '0 3px', borderRadius: '3px' }}>AIza</code>),
                      then add it to <code style={{ background: '#fed7aa', padding: '0 3px', borderRadius: '3px' }}>server_python/.env</code> and restart the server.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="scanner-disease-box" style={{background:sevStyle.bg,borderColor:sevStyle.border}}>
                <span className="scanner-ai-badge" style={{
                  background: result.modelSource?.includes('MobileNet') ? 'linear-gradient(135deg,#3b82f6,#1d4ed8)'
                            : result.modelSource?.includes('Gemini')    ? 'linear-gradient(135deg,#22c55e,#15803d)'
                            : 'linear-gradient(135deg,#f97316,#c2410c)'
                }}>
                  {result.modelSource?.includes('MobileNet') ? '🧠 ML MODEL DETECTED'
                  : result.modelSource?.includes('Gemini')   ? '✨ GEMINI AI DETECTED'
                  : '📋 DEMO DATA'}
                </span>
                {result.modelSource && (
                  <p style={{fontSize:'0.68rem',color:'var(--text-muted)',marginBottom:'0.4rem',fontStyle:'italic'}}>
                    Powered by: {result.modelSource}
                  </p>
                )}
